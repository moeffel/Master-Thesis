...
st.title("🚀 CryptoPro Optimizer AI Dashboard")

# --- Sidebar für Konfigurationen ---
st.sidebar.header("⚙️ Konfigurationen")
NUMBER_OF_TOP_COINS_TO_SCAN = st.sidebar.slider("Anzahl Top Coins (CoinGecko)", 30, 500, 75, 10, help="CoinGecko Top Coins für Analyse (Max 300). Mehr Coins = längere Ladezeit.")
...
PORTFOLIO_ASSET_STOP_LOSS_PCT = st.sidebar.slider("Konzept. Portfolio SL (%)", 0.03, 0.20, 0.10, 0.01, format="%.2f", help="Konzeptioneller Stop-Loss für Assets im Kernportfolio.")
RISK_FREE_RATE = 0.02
...
USE_ATR_FOR_RISK_MANAGEMENT = st.sidebar.checkbox("ATR für taktisches Risiko-Mgmt nutzen?", True, help="Stop-Loss und Positionsgröße basierend auf ATR.")
ATR_PERIOD = 14
ATR_STOP_LOSS_MULTIPLIER = st.sidebar.slider("ATR Stop-Loss Multiplikator", 1.0, 4.0, 2.0, 0.1, help="Vielfaches des ATR für Stop-Loss (wenn ATR aktiv).")
...
FNG_TA_EXTREME_GREED_SELL_THRESHOLD = st.sidebar.slider("F&G Gier-Schwelle für Cash-Erhöhung", 65, 85, 75, 1)
FNG_TA_EXTREME_FEAR_BUY_THRESHOLD = st.sidebar.slider("F&G Angst-Schwelle für Cash-Reduktion", 15, 35, 25, 1)
...
PROFIT_TAKING_THRESHOLD_PCT_CONFIG = st.sidebar.slider("Profit-Taking Schwelle (%) für Hinweis", 20, 200, 50, 10, help="Wenn ein Asset im Portfolio diesen %-Gewinn (ggü. vor 30T) erreicht, wird ein Hinweis zur Gewinnmitnahme angezeigt.") / 100
...
MIN_DATA_POINTS_FOR_ANALYSIS = st.sidebar.slider("Min. Datenpunkte für TA (Tage)", 20, 60, 35, 1, help="Min. Handelstage für TA. Kürzere Historien = höheres Risiko.")
...
def initialize_exchange():
    ...
    if exchange_instance is None:
        try:
            ...
            st.sidebar.error(f"Fehler Börsenverbindung: {e}"); st.error(f"Börsenverbindung: {e}"); st.stop()
...
def get_fear_and_greed_index_data_cached(days=FNG_HISTORY_DAYS):
    ...
    try:
        ...
        current_value, current_classification = np.random.randint(40,60), "Neutral (Simuliert)" # Fallback für aktuellen Wert
    ...
    if df_hist.empty:
        ...
        if days > 0: # Versuche, Dummy-Daten für Plot zu erstellen, wenn Historie gewünscht
        ...
        st.sidebar.warning("F&G API: 'value'-Spalte fehlt in historischen Daten. Chart könnte unvollständig sein.")
    ...
    for coin_data in coins:
        ...
        if market_info and market_info.get('active', False):
            if pair.upper() in PAIRS_TO_EXCLUDE_FROM_OPTIMIZATION and pair.upper() != 'USDC/USDT':
                 print(f"Info: {pair} aktiv, aber auf Ausschlussliste für Optimierung.")
            symbols.append(pair); found_count +=1
    print(f"Gefunden für Scan: {len(symbols)}/{n} Assets auf {TARGET_EXCHANGE} ({quote}) nach Filter.")
    return symbols
...
def fetch_crypto_data_cached(symbols, timeframe='1d', limit=365):
    ...
    status_text.text(f"Daten für {len(data)} Assets geladen."); progress_bar.empty()
...
def optimize_portfolio_sharpe(historical_data_dict, risk_free_rate, assets_for_optimization, max_assets_limit, assets_with_ta_data, coingecko_ids_map, coingecko_categories_cache, current_tickers_dict):
    ...
    if not prices_dict or len(prices_dict) < 2:
        st.sidebar.warning(f"Zu wenig Daten ({len(prices_dict)}) für Optimierung."); return None, None, None, pd.DataFrame()
    ...
    if prices_df.empty or len(prices_df.columns) < 2:
        st.sidebar.warning("Zu wenig Assets nach Bereinigung für Optimierung."); return None, None, None, pd.DataFrame()
    ...
    if final_weights:
        try:
            ...
            if not mu_f.empty and not S_f.empty and not weights_s.empty and len(mu_f) == len(S_f) == len(weights_s):
                ...
                st.sidebar.warning("Konnte Performance des optimierten Portfolios nicht präzise berechnen (Dimensionen mu/S/Gewichte passen nicht).")
                ...
                if weight > 0.0001:
                    ...
                    if asset in assets_with_ta_data and not assets_with_ta_data[asset].empty:
                        ...
                        if 'RSI' in df_asset_ta.columns and not pd.isna(df_asset_ta['RSI'].iloc[-1]):
                            ...
                            if rsi_val_num > 75: current_rsi_val += " (🔥 SEHR Überkauft!)"
                            elif rsi_val_num > 70: current_rsi_val += " (Überkauft)"
                            elif rsi_val_num < 25: current_rsi_val += " (🧊 SEHR Überverkauft!)"
                            elif rsi_val_num < 30: current_rsi_val += " (Überverkauft)"
                        if 'close' in df_asset_ta.columns and len(df_asset_ta['close']) >= 31 and current_price > 0:
                            ...
                            if price_30d_ago > 0:
                                ...
                                if perf_30d > PROFIT_TAKING_THRESHOLD_PCT_CONFIG : perf_30d_val += " (💰 Profit-Taking erwägen!)"
...
def run_monte_carlo_simulation(portfolio_expected_annual_return, portfolio_annual_volatility,
    ...
    if portfolio_expected_annual_return is None or portfolio_annual_volatility is None or portfolio_annual_volatility <= 1e-6 :
        print(f"Warning: Ungültige Eingaben für Monte-Carlo ({title_suffix}).")
        return None, None
    ...
    if daily_volatility <= 1e-8:
        print(f"Warning: Tägliche Vola für MC ({title_suffix}) zu gering.")
        return None, None
    ...
    if create_plot:
        ...
        ax.set_xlabel(f'Portfolio Endwert (Start={initial_portfolio_value})'); ax.set_ylabel('Häufigkeit')
...
def generate_trade_recommendations(top_candidates_dict, current_portfolio_value, risk_profile_trades, optimized_portfolio_assets, assets_with_ta_data, current_tickers_dict):
    ...
    for trade_idea in selected_ideas:
        ...
        if USE_ATR_FOR_RISK_MANAGEMENT and atr_value and atr_value > 1e-8 and direction == "Long":
            trailing_stop_suggestion = f" (Trailing SL: {ATR_STOP_LOSS_MULTIPLIER:.1f} ATR erwägen nach TP1)"
        recommendations_data.append({
            ...
            "Pos.Größe (USD)": f"{pos_size_usd:,.2f}", "Grund": reason,
...
def generate_portfolio_limit_order_suggestions(optimized_assets, assets_with_ta_data, current_tickers_dict, current_portfolio_value, target_weights, min_usd_allocation_per_asset):
    ...
    for asset in optimized_assets: # Iteriere nur über Assets im optimierten Portfolio
        ...
        if target_weight_for_asset == 0: # Nicht im Zielportfolio, überspringen
            continue
        ...
        # Szenario 1: Preis nahe unterem Bollinger Band (max 3% darüber), RSI < 50
        if not pd.isna(latest_ta['BB_low']) and latest_ta['BB_low'] > 0 and \
           ...
           not pd.isna(latest_ta['RSI']) and latest_ta['RSI'] < 50 :
            limit_price = latest_ta['BB_low'] * 1.002 # Knapp über dem Band
            reason = f"Kauf nahe Unt. BB (aktuell {format_price(latest_ta['BB_low'])})"
            if latest_ta['RSI'] < 35: confidence = "Hoch"
        
        # Szenario 2: Rücksetzer zur SMA20 (max 2.5% darüber) in Aufwärtstrend (SMA20 > SMA50), RSI < 60
            ...
            reason = f"Kauf Rücksetzer SMA20 i. Trend (SMA20 {format_price(latest_ta['SMA20'])})"
            if df_asset['SMA20'].iloc[-1] > df_asset['SMA20'].iloc[-5]: # SMA20 steigend
                 confidence = "Hoch"

        # Szenario 3: Stark überverkauft (RSI < 30)
        elif not pd.isna(latest_ta['RSI']) and latest_ta['RSI'] < 30:
            limit_price = current_price * 0.99 # 1% unter aktuellem Preis
            reason = f"Stark überverkauft (RSI {latest_ta['RSI']:.1f})"
            confidence = "Hoch"
        
        # Szenario 4: RSI < 40 und Preis über SMA50 (konservativer), nicht schon durch andere Regeln abgedeckt
             ...
             reason = f"Überverkauft (RSI {latest_ta['RSI']:.1f}) & >SMA50"
             confidence = "Mittel"

        if limit_price and limit_price < current_price and limit_price > 0:
            # Positionsgröße: Ziel ist es, das Delta zur Zielallokation zu füllen,
            ...
            suggestions.append({
                ...
                "Ziel Stück": f"{units_to_buy:.4f}", "Ziel Wert (USD)": f"${order_value_usd:,.2f}",
                "Begründung": reason, "Konfidenz": confidence,
...
def plot_portfolio_allocation_st(allocation_dict, title="Portfolio Allokation", max_items=15):
    if not allocation_dict: st.warning(f"Keine Daten für Plot: {title}"); return None
    ...
    if not lbls or not sizs or sum(sizs) < 1e-5: st.warning(f"Keine Daten für Plot '{title}' (nach Filter)."); return None
    ...
    if not plot_sizs or sum(plot_sizs) < 1e-5: st.warning(f"Keine plotbaren Größen für '{title}'."); return None
...
def plot_correlation_matrix_heatmap_st(corr_matrix, title="Korrelationsmatrix"):
    if corr_matrix is None or corr_matrix.empty: st.warning(f"Keine Daten für Korr-Plot: {title}"); return None
    ...
    if indicators_to_plot:
        ...
        if 'BB' in indicators_to_plot and all(c in df_asset.columns for c in ['BB_high', 'BB_low']) \
           and not df_asset['BB_high'].isnull().all() and not df_asset['BB_low'].isnull().all():
            ax.fill_between(df_asset.index, df_asset['BB_low'], df_asset['BB_high'], color='gray', alpha=0.2, label='Bollinger Bänder')
...
def print_footer_st():
    st.markdown("---")
    st.caption("HINWEIS: Diese Analyse ist experimentell und dient nur zu Informationszwecken. KEINE ANLAGEBERATUNG. Kryptowährungen sind hochvolatil und risikoreich. Das Skript führt KEINE ECHTEN TRADES AUS.")
...
def run_analysis():
    ...
    if st.sidebar.button("🚀 Analyse Starten / Aktualisieren", type="primary", use_container_width=True):
        ...
        with st.spinner("Führe vollständige Analyse durch... Bitte warten. Dies kann einige Minuten dauern."):
            ...
            if ENABLE_DYNAMIC_CASH and fng_hist_for_cash is not None and not fng_hist_for_cash.empty and 'value' in fng_hist_for_cash.columns and fng_hist_for_cash['value'].notna().any():
                ...
                else: st.sidebar.warning("Zu wenig F&G Historiendaten für SMA-Berechnung.")
            ...
            # NEU: Limit-Order-Vorschläge für Portfolio
            ...
            if 'last_opt_weights' in st.session_state and st.session_state.last_opt_weights and st.session_state.opt_weights_current_run:
                ...
                for asset in all_assets_rebal:
                    ...
                    if abs(change_value) > MIN_USD_ALLOCATION_PER_ASSET * 0.05 :
                         rebal_advice.append({"Asset": asset, "Alte Gew.": f"{old_weight:.2%}", "Neue Gew.": f"{new_weight:.2%}", "Neuwert USD": f"${new_value:,.0f}", "Änderung USD": f"${change_value:,.0f}", "Aktion (Stück)": f"{'KAUFEN' if change_value > 0 else 'VERKAUFEN'}: {change_units_str}"})
    ...
    if st.session_state.get('results_ready', False) and not st.session_state.get('analysis_in_progress', False):
        st.header(f"📊 Ergebnisse der Analyse")
        ...
        with col_sum2:
            st.subheader("⭐ Performance des optimierten Portfolios")
            ...
            if opt_perf_sum and opt_perf_sum.get("annual_return") is not None and opt_w_sum:
                ...
                st.metric("Progn. Jährl. Rendite (annualisiert)", f"{opt_perf_sum.get('annual_return', 0):.2%}")
                st.metric("Progn. Jährl. Volatilität (annualisiert)", f"{opt_perf_sum.get('annual_volatility', 0):.2%}")
        ...
        if st.session_state.get('opt_asset_details_df') is not None and not st.session_state.opt_asset_details_df.empty:
            st.subheader("🎯 Optimiertes Zielportfolio - Details & Projektinfos")
            ...
            else: st.warning("Spalte 'Asset' für Detailansicht nicht gefunden.")
            with st.expander("Hinweis zur Coin-Wahl im optimierten Portfolio"):
                st.markdown("""Die Auswahl und Gewichtung der Assets basiert auf der Maximierung der erwarteten risikoadjustierten Rendite (Sharpe Ratio). Faktoren für höhere Gewichtung: Höhere erw. Rendite (ggf. angepasst durch Vola-Penalty), geringere Volatilität, geringe/negative Korrelation zu anderen Assets. Assets mit extrem hoher relativer Volatilität (normalisierter ATR basierend auf aktuellem Preis) erhalten einen Malus auf ihre erwartete Rendite. Sektoren/Beschreibungen: CoinGecko.""")
        st.markdown("---")

        if st.session_state.get('opt_assets_list') and st.session_state.get('assets_with_ta_data'):
            st.subheader("📈 Charts der optimierten Portfolio-Assets")
                        ...
                        with current_col_opt:
                            ...
                            if asset_symbol_opt in st.session_state.assets_with_ta_data:
                                ...
                                if not df_for_plot_opt.empty:
                                    ...
                                    with st.expander(f"Chart für {asset_symbol_opt} (TA: {ta_signal_opt_asset})", expanded=False):
                            ...
                            else: st.caption(f"Keine Chartdaten für Portfolio-Asset {asset_symbol_opt} gefunden.")
            st.markdown("---")

        st.subheader("😨 Fear & Greed Index Verlauf & TA")
        ...
        if fng_hist_df_for_plot is not None and not fng_hist_df_for_plot.empty and 'value' in fng_hist_df_for_plot.columns and fng_hist_df_for_plot['value'].notna().any():
            ...
            else: st.info("Zu wenig historische F&G-Daten für SMA-Berechnung. Zeige Rohdaten oder simulierte Historie.")
            ...
            if fig_fng: st.pyplot(fig_fng); plt.close(fig_fng)
                ...
                ax_fng_raw.set_title("Fear & Greed Index (Rohdaten/Simulierte Historie)"); ax_fng_raw.set_ylabel("Index Wert"); ax_fng_raw.legend(loc='upper left'); ax_fng_raw.grid(True, ls=':', alpha=0.7); fig_fng_raw.autofmt_xdate(); ax_fng_raw.xaxis.set_major_formatter(mdates.DateFormatter('%d.%m')); st.pyplot(fig_fng_raw); plt.close(fig_fng_raw)
                 ...
                 st.info("Fear & Greed Index Chart nicht verfügbar (Historie nicht geladen oder 'value'-Spalte fehlt trotz Fallback).")
        else: st.info("Fear & Greed Index Chart nicht verfügbar (Historie nicht geladen, 'value'-Spalte fehlt oder enthält keine gültigen Daten).")
        st.markdown("---")

        if st.session_state.get('rebalancing_advice_df') is not None and not st.session_state.rebalancing_advice_df.empty:
            st.subheader("🔄 Rebalancing Vorschläge (Vergleich zu vorheriger Optimierung)")
        ...
        if 'trade_recs_df' in st.session_state and not st.session_state.trade_recs_df.empty:
            st.subheader("💡 Taktische Trade-Vorschläge (Neue Positionen)")
            ...
            st.write("**Charts zu den taktischen Trade-Vorschlägen:**")
                        ...
                        with current_col:
                            ...
                            if asset_symbol_trade in st.session_state.assets_with_ta_data:
                                ...
                                if not df_for_plot.empty:
                                    with st.expander(f"Chart für {asset_symbol_trade} ({rec['Richtung']}) - {rec['Grund']}", expanded=False):
                                        fig_trade_asset = plot_asset_chart_st(df_for_plot.tail(120), asset_symbol_trade, indicators_to_plot=['SMA20', 'SMA50', 'BB'], title_prefix=f"{rec['Richtung']} Signal für")
                                        if fig_trade_asset: st.pyplot(fig_trade_asset); plt.close(fig_trade_asset)
                                        else: st.caption("Chart konnte nicht erstellt werden.")
                            else: st.caption(f"Keine Chartdaten für {asset_symbol_trade} gefunden.")
        else: st.info("Keine überzeugenden taktischen Trade-Vorschläge gefunden.")
        st.markdown("---")
        
        # NEU: Anzeige Limit-Order-Vorschläge
        if 'portfolio_limit_order_suggestions_df' in st.session_state and not st.session_state.portfolio_limit_order_suggestions_df.empty:
            st.subheader("📈 Limit-Order Kaufvorschläge zur Portfolio-Optimierung (Tagesbasis)")
            ...
            cols_to_show_limit = ["Aktion", "Limit Preis", "Akt. Preis", "Ziel Stück", "Ziel Wert (USD)", "Begründung", "Konfidenz", "TV Chart"]
            ...
            st.caption("Hinweis: Diese Limit-Vorschläge basieren auf Tages-Chart-Analysen (z.B. Kauf bei Rücksetzern zu Unterstützungen wie unteres Bollinger Band oder SMA20). Sie sind für eine schrittweise Optimierung des Zielportfolios gedacht, nicht für hochfrequentes Daytrading, da keine Intraday-Daten verwendet werden.")
        else: st.info("Keine spezifischen Limit-Order Kaufvorschläge zur Portfolio-Optimierung auf Tagesbasis gefunden.")
        st.markdown("---")


        if st.session_state.get('opt_weights_current_run') and st.session_state.get('prices_df_discrete') is not None:
            st.subheader("💰 Diskrete Allokation (Rebalancing zum Zielportfolio)")
            try:
                ...
                st.write(f"**Kapital für Rebalancing (investierter Teil, nach dyn. Cash):** ${invest_capital:,.2f}")
                ...
                if temp_sum_w > 0:
                    ...
                    st.write(f"Assets nach Mindestfilter ({MIN_USD_ALLOCATION_PER_ASSET}$): {len(norm_discrete_w)}, Kapital dafür: ${actual_capital_discrete:,.2f}")
                    if norm_discrete_w and actual_capital_discrete > 1 :
                        ...
                        if missing_prices_assets:
                            st.warning(f"Fehlende Preise für diskrete Allokation: {', '.join(missing_prices_assets)}. Diese Assets werden ignoriert.")
                            norm_discrete_w = {k:v for k,v in norm_discrete_w.items() if k in prices_for_da_final}
                            if not norm_discrete_w: raise ValueError("Keine Assets mit Preisen für diskrete Allokation.")
                        if not norm_discrete_w: st.info("Keine Assets nach Preisfilterung für diskrete Allokation.")
                        else:
                            ...
                            for sym, num in allocation.items():
                                ...
                                if price_for_val and price_for_val > 0:
                                    ...
                                    alloc_data_discrete.append({"Asset": sym, "Stückzahl": f"{num:.4f}", "Zielwert (USD)": f"${val:,.2f}"})
                    ...
                    else: st.info("Keine Assets nach Filter/Normalisierung oder zu wenig Kapital für diskrete Allokation.")
                else: st.info(f"Keine Assets erfüllen Mindestallokation von {MIN_USD_ALLOCATION_PER_ASSET}$.")
        ...
        if opt_perf_mc and opt_perf_mc.get("annual_return") is not None and opt_perf_mc.get("annual_volatility") is not None :
            st.subheader("🎲 Monte Carlo Simulationen des optimierten Portfolios")
        ...
        else: st.info("Monte-Carlo nicht möglich (Performance-Daten des optimierten Portfolios fehlen oder sind ungültig).")
        st.markdown("---")
        ...
        with col_wd:
            if 'weekday_df' in st.session_state and st.session_state.weekday_df is not None and not st.session_state.weekday_df.empty:
                st.subheader("📅 Hist. Wochentags-Performance"); st.dataframe(st.session_state.weekday_df.style.format("{:.2f}%", na_rep="N/A"))
            ...
            else: st.info("Keine Daten für Wochentagsanalyse.")
        with col_corr:
            st.subheader("🔗 Korrelationsmatrix"); corr_m_plot = st.session_state.get('corr_matrix')
            ...
            else: st.info("Keine Daten für Korrelationsmatrix.")
        st.markdown("---")

        st.subheader("🥧 Zielportfolio Allokation"); opt_w_plot = st.session_state.get('opt_weights_current_run')
        if opt_w_plot:
            ...
            else: st.info("Keine signifikanten Gewichte im optimierten Portfolio für Plot.")
        else: st.info("Kein optimiertes Portfolio für Plot.")
        print_footer_st()
    elif st.session_state.get('analysis_in_progress', False): pass
    else: st.info("Willkommen! Bitte passe die Konfigurationen in der Sidebar an und klicke auf '🚀 Analyse Starten / Aktualisieren', um die Analyse zu beginnen.")
